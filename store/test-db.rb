...
#########################################
  ...
  def test_invoice_needs_shipment
    res = DB.exec("select * from invoice_needs_shipment(1)")
    assert_equal 't', res[0]['invoice_needs_shipment']
    res = DB.exec("select * from invoice_needs_shipment(2)")
    assert_equal 't', res[0]['invoice_needs_shipment']
    res = DB.exec("select * from invoice_needs_shipment(3)")
    assert_equal 'f', res[0]['invoice_needs_shipment']
    res = DB.exec("select * from invoice_needs_shipment(4)")
    assert_equal 'f', res[0]['invoice_needs_shipment']
    res = DB.exec("select * from invoice_needs_shipment(99)")
    assert_equal 'f', res[0]['invoice_needs_shipment']
  end

  def test_shipcost
    res = DB.exec("select * from shipcost('US', 0)")
    assert_equal '0', res[0]['shipcost']
    res = DB.exec("select * from shipcost('CA', 0)")
    assert_equal '0', res[0]['shipcost']
    res = DB.exec("select * from shipcost('ZH', 0)")
    assert_equal '0', res[0]['shipcost']
    res = DB.exec("select * from shipcost('US', 0.1)")
    assert_equal '3', res[0]['shipcost']
    res = DB.exec("select * from shipcost('US', 1)")
    assert_equal '4', res[0]['shipcost']
    res = DB.exec("select * from shipcost('US', 1.5)")
    assert_equal '5', res[0]['shipcost']
    res = DB.exec("select * from shipcost('US', 4)")
    assert_equal '7', res[0]['shipcost']
    res = DB.exec("select * from shipcost('US', 4.01)")
    assert_equal '12', res[0]['shipcost']
    res = DB.exec("select * from shipcost('CA', 0.3)")
    assert_equal '5', res[0]['shipcost']
    res = DB.exec("select * from shipcost('CA', 1)")
    assert_equal '6', res[0]['shipcost']
    res = DB.exec("select * from shipcost('CA', 400)")
    assert_equal '13', res[0]['shipcost']
    res = DB.exec("select * from shipcost('RU', 0.5)")
    assert_equal '7', res[0]['shipcost']
    res = DB.exec("select * from shipcost('IE', 1)")
    assert_equal '8', res[0]['shipcost']
    res = DB.exec("select * from shipcost('IE', 1.01)")
    assert_equal '9', res[0]['shipcost']
    res = DB.exec("select * from shipcost('SG', 400)")
    assert_equal '14', res[0]['shipcost']
    res = DB.exec("select * from shipcost('XX', 1000)")
    assert_equal '1000', res[0]['shipcost']
    res = DB.exec("select * from shipcost('XX', -1)")
    assert_equal '1000', res[0]['shipcost']
  end

  def test_invoice_shipcost
    res = DB.exec("select * from invoice_shipcost(1)")
    assert_equal '6', res[0]['cost']
    res = DB.exec("select * from invoice_shipcost(2)")
    assert_equal '10', res[0]['cost']
    res = DB.exec("select * from invoice_shipcost(3)")
    assert_equal '0', res[0]['cost']
    res = DB.exec("select * from invoice_shipcost(4)")
    assert_equal '0', res[0]['cost']
  end

  def test_cart_get
    res = DB.exec("select * from cart_get_id(1)")
    assert_nil res[0]['id']
    res = DB.exec("select * from cart_get_id(6)")
    assert_nil res[0]['id']
    res = DB.exec("select * from cart_get_id(7)")
    assert_equal '4', res[0]['id']
  end

  def test_cart_new
    res = DB.exec("select * from cart_new_id(1)")
    assert_equal '5', res[0]['id']
    res = DB.exec("select * from invoices where id = 5")
    assert_equal '5', res[0]['id']
